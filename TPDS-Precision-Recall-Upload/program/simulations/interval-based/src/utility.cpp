...
void Utility::saveFPRH(vector<FalsePositiveRateHistory> fh, ofstream &os){
    ...
    os << "#    base_interval_length: " << base_interval_length << endl;

    os << "# absolute_time   ";
    for(int i = 0; i < FPRH_NUM; i++){
        os << "eps(" << epsilonList.at(i) << "):totalTrueSoFar   ";
        os << "eps(" << epsilonList.at(i) << "):totalSoFar   ";
        os << "eps(" << epsilonList.at(i) << "):fps   ";
        
    }
    os << endl;

    
    for(int step = 0; step < run_up_to; step++){
        os.width(8);
        os << step << "  ";
        
        for(int i = 0; i < FPRH_NUM; i++){
            os.width(5);
            os << fh.at(i).history.at(step).totalTrueSoFar << "  ";
            os.width(5);
            os << fh.at(i).history.at(step).totalSoFar << "  ";
            os.width(8);
            os << fh.at(i).history.at(step).fpr << "  ";
...
void Utility::clearFPRH(vector<FalsePositiveRateHistory> fh){
    for(int i = 0; i < FPRH_NUM; i++){
        fh.at(i).history.clear();
    }
...
int Utility::GetRandomNumberInRange(
  ...
  int b   // range upper bound
	...
	int result = distx(mt);
	//printf("random %d\n", result);
	//	if (result > 100000) printf("Warning: random number produces negative number %d, [a,b]=[%d,%d] \n",result,a,b);
	return result;
}

//
//  GetRandomNumberGeometricDistribution() - return a random positive number following geometric distribution
//  based on example code from www.cplusplus.com/reference/random/geometric_distribution
//

std::default_random_engine Utility::generator;

int Utility::GetRandomNumberGeometricDistribution(double p){
    ...
    return num + 1; // returned value should be >= 1
...
adjacency_matrix Utility::ReadGraph(const string& topo) {
  ...
  if (topo == "CompleteGraph") {
	  ...
	  ifstream in(topo);
	  if (!in.is_open()) { printf("Can't read the topology.graph file.\n"); exit(1); }
	  string content;
...
probability_matrix Utility::ReadProbabilityMatrix(const string& prob) {
  ...
  if (prob == "Random") {
    ...
    for (int i = 0; i < number_of_processes; i++) {
	    ...
	    for (int j = 0; j < number_of_processes; j++)
	    {
		    if (j/num_members_in_cluster == q) one_row.push_back(prob_incluster);
		    else one_row.push_back(prob_outcluster);
	    }
	    P.push_back(one_row);
    }
  }
	  ...
	  ifstream in(prob);
	  if (!in.is_open()) { printf("Can't read the probability_matrix.in file.\n"); exit(1); }
	  string content;
	  while (getline(in, content)) {
		  vector<string> split_comma = split(content, ',');
		  vector<double> one_row;
...
int Utility::GetIndexOfWeightedNormalDistribution(double std_percent, int size = number_of_processes) {
  ...
  double val = distribution(generator);
  if (val <= 0) return 0;
  if (val >= number_of_processes-1) return number_of_processes-1;
