...
private:
  ...
  std::queue<Candidate> candidateQueue; // queue of candidates to be proposed in Garg's algorithm
...
public:
  // Constructor
    ...
    // and active_size = 1
    augmented_time = HVC(vector<int>(number_of_processes, -1*epsilon-1),1); 
  ...
  void insertNewCandidate(const Candidate & newCand);
