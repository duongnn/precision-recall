...
void Candidate::print(ofstream &os){
  ...
  os << ", ";
  os.width(6); os << candId << "):" << std::flush;
...
void Token::printToken(ofstream & os){
  ...
  // compute maximum physical time difference between all pairs of processes
  
  // next is the maximum record_of_my_time_now difference between all pairs of processes
  ...
  os << "maximum physical time difference between all pairs of processes : " << (max - min) << endl;
